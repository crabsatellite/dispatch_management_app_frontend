...
import {
  DELIVERY_STATE,
  DISPATCHER_START_LOCATION_KEY,
  DISPATCHER_TYPE,
} from "../../../../utils/delivery_plan_utils";
...
const PackagePickupStep = ({
  ...
  if (deliveryState === DELIVERY_STATE.PICKUP_PROCESSING) {
    return (
      <Card title="PICK-UP PROCESSING" style={{ width: 1000, left: 20 }}>
        <Result
        ...
        <Image
          ...
          width={200}
          src={
            dispatcher === DISPATCHER_TYPE.ROBOT ? "./robot.png" : "./drone.png"
          }
        />
      </Card>
    );
    ...
    return (
      <Card title="PICK-UP PLANNING" style={{ width: 1000, left: 20 }}>
        <Collapse bordered={false} defaultActiveKey={["1"]}>
          ...
          <Panel header="SELECT WAREHOUSE" key="1">
            <Dropdown.Button
              style={{ marginLeft: 410, top: 100 }}
              type="primary"
              menu={startLocationMenuProps}
              onClick={handleStartLocationMenuClick}
            >
              Warehouse
            </Dropdown.Button>
          ...
          <Panel header="RESET HERE" key="1">
            <Button
              type="primary"
              onClick={() => {
                setDeliveryState(DELIVERY_STATE.RESET_ROUTE);
              }}
            >
              Reset
            </Button>
          ...
          <Panel header="DISPATCHER / WAREHOUSE CONFIRMATION" key="1">
            ...
            <p>
              <InfoCircleFilled />
              ONCE CLICK, WE WILL PREPRAE THE DISPATCHER FROM SELECTED WAREHOUSE
              SOON
            </p>
          </Panel>
        </Collapse>
    ...
    return (
      <Card title="PICK-UP ONBOARDING" style={{ width: 1000, left: 20 }}>
        <Collapse bordered={false} defaultActiveKey={["1"]}>
          ...
          <Panel type="primary" header="PICK-UP DISPATCH" key="1">
            <Button
              type="primary"
              onClick={() => {
                setDeliveryState(DELIVERY_STATE.PICKUP_PROCESSING);
              }}
            >
              Start Pick-up
            </Button>
